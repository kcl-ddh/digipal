...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding M2M table for field owners on 'ItemPart'
        db.create_table('digipal_itempart_owners', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('itempart', models.ForeignKey(orm['digipal.itempart'], null=False)),
            ('owner', models.ForeignKey(orm['digipal.owner'], null=False))
        ))
        db.create_unique('digipal_itempart_owners', ['itempart_id', 'owner_id'])


        # Changing field 'ItemPart.current_item'
        db.alter_column('digipal_itempart', 'current_item_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['digipal.CurrentItem'], null=True))

    def backwards(self, orm):
        # Removing M2M table for field owners on 'ItemPart'
        db.delete_table('digipal_itempart_owners')


        # User chose to not deal with backwards NULL issues for 'ItemPart.current_item'
        raise RuntimeError("Cannot reverse this migration. 'ItemPart.current_item' and its values cannot be restored.")
