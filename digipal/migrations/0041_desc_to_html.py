...
def get_html_from_html_field(val):
    "returns html in a string from the value of a HTML field. Convert plain text to html if necessary."
    import re
    ...
    if ret:
        is_xml = (val[0] == u'<')
        if not is_xml:
            # convert reserved chars to entities
            ret = escape(ret)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # We convert all the digipal.Description.description into HTML because we are now using TinyMCE
        # Without this conversion we would loose line breaks which need to be converted into paragraphs.
        
        for desc in orm['digipal.Description'].objects.all():
            if desc.description:
                desc.description = get_html_from_html_field(desc.description)
                desc.save()
