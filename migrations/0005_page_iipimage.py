...
from digipal.models import normalize_string
...
class Migration(DataMigration):
    ...
    def path(self, page):
        ...
        repository = page.item_part.current_item.repository.short_name
        shelfmark = page.item_part.current_item.shelfmark

        if repository and shelfmark:
            repository = normalize_string(repository)
            shelfmark = normalize_string(shelfmark)
            locus = normalize_string(page.locus)

            ret = u'%s/%s/%s/%s.%s' % (settings.IMAGE_SERVER_WEB_ROOT,
