...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding unique constraint on 'CatalogueNumber', fields ['source', 'text', 'number', 'historical_item']
        db.create_unique('digipal_cataloguenumber', ['source_id', 'text_id', 'number', 'historical_item_id'])

        # Adding unique constraint on 'Description', fields ['source', 'text', 'historical_item']
        db.create_unique('digipal_description', ['source_id', 'text_id', 'historical_item_id'])

        # Adding field 'Text.legacy_id'
        db.add_column('digipal_text', 'legacy_id',
                      self.gf('django.db.models.fields.IntegerField')(null=True, blank=True),
                      keep_default=False)

        # Adding field 'Text.date'
        db.add_column('digipal_text', 'date',
                      self.gf('django.db.models.fields.CharField')(max_length=128, null=True, blank=True),
                      keep_default=False)

        # Adding field 'Text.url'
        db.add_column('digipal_text', 'url',
                      self.gf('django.db.models.fields.URLField')(max_length=200, null=True, blank=True),
                      keep_default=False)

        # Adding M2M table for field categories on 'Text'
        db.create_table('digipal_text_categories', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('text', models.ForeignKey(orm['digipal.text'], null=False)),
            ('category', models.ForeignKey(orm['digipal.category'], null=False))
        ))
        db.create_unique('digipal_text_categories', ['text_id', 'category_id'])

        # Adding M2M table for field languages on 'Text'
        db.create_table('digipal_text_languages', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('text', models.ForeignKey(orm['digipal.text'], null=False)),
            ('language', models.ForeignKey(orm['digipal.language'], null=False))
        ))
        db.create_unique('digipal_text_languages', ['text_id', 'language_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Description', fields ['source', 'text', 'historical_item']
        db.delete_unique('digipal_description', ['source_id', 'text_id', 'historical_item_id'])

        # Removing unique constraint on 'CatalogueNumber', fields ['source', 'text', 'number', 'historical_item']
        db.delete_unique('digipal_cataloguenumber', ['source_id', 'text_id', 'number', 'historical_item_id'])
        ...
        # Deleting field 'Text.legacy_id'
        db.delete_column('digipal_text', 'legacy_id')

        # Deleting field 'Text.date'
        db.delete_column('digipal_text', 'date')

        # Deleting field 'Text.url'
        db.delete_column('digipal_text', 'url')

        # Removing M2M table for field categories on 'Text'
        db.delete_table('digipal_text_categories')

        # Removing M2M table for field languages on 'Text'
        db.delete_table('digipal_text_languages')
