...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # The ItemOrigin model now uses foreign keys directly to Place and Institution 
        # instead of the GenericForeignKey
        #
        # We convert all the data to the new schema
        # 
        ...
        for item_origin in orm['digipal.itemorigin'].objects.all():
            obj = orm['%s.%s' % (item_origin.content_type.app_label, item_origin.content_type.name)].objects.get(id=item_origin.object_id)
            
            if obj:
                setattr(item_origin, item_origin.content_type.name, obj)
                
            #print item_origin.id, obj.__class__.__name__, obj.id
            item_origin.save()
