...
def set_description(self, source_name, description, orm):
    ...
    sources = orm['digipal.Source'].objects.filter(name=source_name)
    source = None
    if sources:
        source = sources[0]
    else:
        source = orm['digipal.Source'](name=source_name, label=source_name.upper())
        source.save()
    ...
    if hand_description is None:
        hand_description = orm['digipal.HandDescription'](hand=self, source=source)
